 ...
 */
class UTPickupFactory_NinjaHealthLarge extends UTHealthPickupFactory placeable ClassGroup(Team1Game);

/** list of adjacent vials; used to adjust AI ratings for vial groups */
var array<UTPickupFactory_NinjaHealthLarge> AdjacentVials;
 ...
 */
function FindAdjacentVials(out array<UTPickupFactory_NinjaHealthLarge> InAdjacentVials, UTPickupFactory_NinjaHealthLarge InitialCaller)
{
	local int i;
	local UTPickupFactory_NinjaHealthLarge OtherVial;

	for (i = 0; i < PathList.length; i++)
	{
		if (PathList[i] != None && PathList[i].Distance < 150 && AdvancedReachSpec(PathList[i]) == None)
		{
			OtherVial =UTPickupFactory_NinjaHealthLarge(PathList[i].GetEnd());
...
defaultproperties
{
	...
	HealingAmount=70//change this to set value
	...
	Begin Object Name=HealthPickUpMesh
		StaticMesh=StaticMesh'NinjaHealthPackage.HealthPackLarge'
